...
def index():
    ...
    stock = db.execute(
        "SELECT symbol, name, price, SUM(shares) as Tshares FROM trans WHERE user_id = ? GROUP BY symbol",
        user_id,
    )
...
def buy():
    ...
    if request.method == "POST":
        ...
        if money < totalprice:
            ...
            db.execute(
                "UPDATE users SET cash = ? WHERE id = ?", money - totalprice, user_id
            )
            db.execute(
                "INSERT INTO trans (user_id, name, shares, price, type, symbol) VALUES(?, ?, ?, ?, ?, ?)",
                user_id,
                itemname,
                shr,
                itemprice,
...
def quote():
    ...
    if request.method == "POST":
        ...
        return render_template(
            "quoted.html", stock=lookup(request.form.get("symbol")), uf=usd
        )
...
def sell():
    ...
    if request.method == "POST":
        ...
        ss = db.execute(
            "SELECT shares FROM trans WHERE user_id = ? AND symbol = ? GROUP BY symbol",
            session["user_id"],
            request.form.get("symbol"),
        )[0]["shares"]

        if ss < int(request.form.get("shares")):
            return apology("You don't have enough amount of share!")

        sele = db.execute("SELECT cash FROM users WHERE id = ?", session["user_id"])[0][
            "cash"
        ]
        db.execute(
            "UPDATE users SET cash = ? WHERE id = ?", sele + pp, session["user_id"]
        )
        db.execute(
            "INSERT INTO trans (user_id, name, shares, price, type, symbol) VALUES (?, ?, ?, ?, ?, ?)",
            session["user_id"],
